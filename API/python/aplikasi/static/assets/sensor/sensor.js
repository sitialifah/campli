...
(async function () {
  ...
  let xhrResponse = await xhrRequest("get", "json", "/ref_sensor/daftar")
  console.log("XHR")
  ...
  for (let i = 0; i < data.length; i++) {
    optionsList += "<option name=id_ref_sensor value='" + data[i]['id'] + "'> " + titleCase(data[i]['nama_sensor']) + " </option>"
  }

  $('#select-sensor').append(optionsList);
